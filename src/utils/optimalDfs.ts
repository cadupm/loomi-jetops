...
async function optimalDfs(graph: Map<string, IFlight[]>, start: string, end:string, start_date:Date, end_date:Date, path = [], paths = []): Promise<IPossiblePaths> { 
  ...
  for (const flight of flights) {
    ...
    const addingWaitingTime = addMinutes(parseISO(flight.departure_date), turn_around)
    const calculatedNewStartDate = addMinutes(addingWaitingTime, duration)

    const isValidFlight = isAfter(parseISO(flight.departure_date), start_date) && isAfter(end_date, calculatedNewStartDate)
      ...
      if(isValidFlight) {
        ...
        optimalDfs(graph, flight.arrival, end, calculatedNewStartDate, end_date, path, paths)
      }
  }
